    ...
    return data.map(item => {
      ...
      // If we only have hour, create date and timestamp based on timeframe
        ...
        // For 30d, distribute across the past 30 days
          ...
          console.log(`[Admin] Processing 30d data for hour: ${hour}`);
          // Use a more even distribution across 30 days
          // Create a deterministic but varied distribution
          const dayOffset = Math.min(29, (hour * 7 + Math.floor(hour / 3)) % 30);
          const targetDate = new Date(now.getTime() - dayOffset * 24 * 60 * 60 * 1000);
          date = targetDate.toISOString().split('T')[0];
          ...
          console.log(`[Admin] 30d data point: hour=${item.hour} -> dayOffset=${dayOffset}, date=${date}, timestamp=${timestamp}`);
        } 
